...
{
    [TestFixture]
    public class add_audio_language_verification_to_episode_filesFixture : MigrationTest<add_audio_language_verification_to_episode_files>
    {
        [Test]
        public void should_add_nullable_column_and_keep_existing_rows()
        {
            ...
            var rows = db.Query<EpisodeFile218>("SELECT \"Id\", \"AudioLanguageVerification\" FROM \"EpisodeFiles\"").ToList();

            rows.Should().HaveCount(1);
            rows.First().AudioLanguageVerification.Should().BeNull();
        }

        private class EpisodeFile218
        {
